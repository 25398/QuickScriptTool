...
| 验收 1 | 独立逐项实机复跑（不采信自述） | 5/6 项属实；抓到 P0 并修复；另修 1 项 P1 工具缺陷 |
| 第 2 轮 | A 段（闭合 P0）+ B 段（依赖倒置、删链接桩、首个引擎执行测试）+ C 段（桥接契约） | ✅ **665 用例 / 0 失败**；引擎可独立链接；桥接命令面双向锁定 |
| 验收 2/3 | 逐项实机复跑 + 发版工具链/FakeFocus32 验收 | 抓到 F1–F4；A4/A5 由验收会话闭合 |
| 第 3 轮 | 清账（F2/F3/F4）+ 分批提交 5 个 + 首次 CI + D 段 D1 | ✅ **672 用例 / 0 失败**；CI 抓到并修复 1 项 P1 仓库缺陷；真实脚本往返基线零差异 |

> 第 3 轮的详细记录（含每一步的踩坑与验证留痕）见 [`docs/refactor-progress.md`](refactor-progress.md) §九。

---

## 一·补：第 3 轮要点（2026-09-19）

| 项 | 结论 |
|----|------|
| **F3 版本号单源化** | 报告列 4 处，实际修 **5 处** —— 报告漏了 `resources/QuickScriptTool.rc` 的 4 个硬编码版本号（发版脚本原本不写它 → 下次发版 exe 属性里版本会是旧的）。7 条写入规则现集中在 `$VersionWrites`，DryRun 与实际执行共用 |
| **F4 DryRun 校验** | DryRun 现在校验全部写入规则（文件存在 + 正则可匹配），把「文件结构变了」提前暴露 |
| **F2 窗口模式抖动** | 命名用例 `background_minimized_quiet_restore` 由 40% 失败 → **0 失败**（15 次中 14 次全绿）。判定拆成 `stolen`（目标窗成前台=缺陷，硬失败）与 `settled`；瞬时 NULL/第三方窗不计失败 |
| **F2 报告漏项** | `background_click_keeps_foreground` 同根因，但我的改造使其变成 0/12 → **已完整还原**，仅记录不修（本机无法可靠 A/B 前台类抖动） |
| **分批提交** | 5 个提交：`52a97c7` 卫生+文档 / `87c37df` 重构主体 / `89053c7` 发版入口+文案 / `6306214` nlohmann 入库修复 / `16619fa` D1。已推送 `a44c267..16619fa` |
| **首次 CI 抓到 P1** | `src/third_party/nlohmann/json.hpp`（920KB）**从未入库**（被 `.gitignore` 的 `third_party/` 连带忽略）→ 干净克隆编译不出产品。已修。另：`Setup OpenCV` 在 runner 上首次跑通 |
| **D 段 D1** | 新增 `tools/script_serialization_selftest.cpp`；**14 个真实脚本往返零差异**；唯一系统性差异 `captureSize` 是设计如此（盖上保存机器的屏幕尺寸），已单独计数并在文档中标注为 D3 的约束 |

### 下一步（D 段 D2–D4）

D2 三处入口收敛到 nlohmann → D3 加 `"v"` + `MigrateAction` → D4 扩充
`ScriptSerializationSelfTest` 并做**变异测试**验证它真能变红。
硬要求不变：每步都必须用真实历史脚本回归（D1 的 `--corpus` 已就位）。
